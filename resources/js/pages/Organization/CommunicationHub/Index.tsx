...
import HubSegmentedTabs, { type HubTab } from '@/components/communication-hub/HubSegmentedTabs'
import HubEmptyState from '@/components/communication-hub/HubEmptyState'
import HubAudienceSettings, { type HubSettingsForm } from '@/components/communication-hub/HubAudienceSettings'
import {
...
const breadcrumbs: BreadcrumbItem[] = [
  { title: 'Announcements & Discussion', href: '/organization/communication-hub' },
]
...
const INFO_BLOCKS = [
  {
    icon: Megaphone,
    title: 'Announcements',
    body: 'Official updates posted only by the organization and board staff.',
  },
  {
    icon: MessageSquare,
    title: 'Discussion Board',
    body: 'Supporters start threads and reply when your access settings allow it.',
  },
  {
    icon: Shield,
    title: 'Access control',
    body: 'Use Access settings to choose who can view and who can post.',
  },
] as const
...
export default function CommunicationHubIndex({
  ...
  const canManageSettings = Boolean(permissions.can_manage_settings ?? permissions.can_manage_announcements)
        ...
        <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="min-w-0">
            ...
            <p className={ch.subheading}>
              Organization announcements and supporter Discussion Board for {organization.name}
            </p>
          </div>
          <HubSegmentedTabs
            value={tab}
            ...
            className="sticky top-0 z-10 sm:static"
          />
        </header>

        {tab === 'settings' && canManageSettings && settings ? (
          <HubAudienceSettings settings={settings} organizationName={organization.name} />
        ) : (
          <>
