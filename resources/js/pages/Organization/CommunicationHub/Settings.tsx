...
export default function CommunicationSettings({ organization, settings, visibilityAudiences }: Props) {
  const audiences = visibilityAudiences ?? {
  ...
  const toggles: Array<{ key: keyof Omit<typeof form.data, 'auto_archive_days' | 'announcement_visibility' | 'discussion_visibility'>; label: string; help: string }> = [
  ...
  return (
    <SettingsLayout activeTab="communication-hub" pageTitle="Announcements & Discussion" pageSubtitle={`Who can view and post for ${organization.name}`}>
      <Head title="A&D Board Settings" />
      ...
      <form
        ...
        className={cn(ch.card, 'space-y-6 p-5 sm:p-6')}
      >
        <section className="space-y-4">
          <div>
            <h2 className="text-sm font-semibold text-foreground">Who can view</h2>
            <p className="mt-0.5 text-xs text-muted-foreground">
              Organizations control audience for Announcements and the Discussion Board separately.
            </p>
          </div>

          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="announcement_visibility">Announcements</Label>
              <select
            ...
            <div className="space-y-2">
              <Label htmlFor="discussion_visibility">Discussion Board</Label>
              <select
                id="discussion_visibility"
                className={cn('flex h-10 w-full rounded-lg border border-input bg-background px-3 text-sm', ch.focus)}
        ...
        <section className="space-y-3">
          <div>
            <h2 className="text-sm font-semibold text-foreground">Who can post</h2>
            <p className="mt-0.5 text-xs text-muted-foreground">
